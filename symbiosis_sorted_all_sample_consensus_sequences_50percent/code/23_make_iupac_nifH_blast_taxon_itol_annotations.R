...
# Step 23: Create iTOL annotations for the mixed-IUPAC nifH sample tree using BLAST taxon labels.
#
# This is the mixed-tree version of Step 21. It annotates the mixed-aware sample tree
# with closest BLAST genus/taxon and BLAST quality metrics. Because mixed-IUPAC
# sequences may contain ambiguity codes, interpret these labels as closest-reference
# labels, not confirmed organism identities.
...
tree_file <- file.path(project_dir, "result/trees/nifH_consensus50_iupac_all_pass.treefile")
blast_file <- file.path(project_dir, "result/tables/iupac_nifH_best_reference_hit_with_taxon.tsv")
out_dir <- file.path(project_dir, "result/itol_annotations_blast_taxon_iupac")
...
stop_if_missing(tree_file, "Mixed-IUPAC tree file")
stop_if_missing(blast_file, "Mixed-IUPAC BLAST best-hit table")
...
required_cols <- c(
  ...
  "percent_identity", "query_coverage_percent", "reference_coverage_percent"
)
...
write_colorstrip <- function(path, dataset_label, values, color_map) {
  ...
  if (!"Unassigned" %in% names(color_map)) color_map <- c(color_map, Unassigned = "#bdbdbd")
...
write_gradient <- function(path, dataset_label, values, color_min, color_max) {
  ...
  writeLines("DATA", con)
  for (i in seq_along(tree_tips)) {
    if (!is.na(values[i])) writeLines(paste(tree_tips[i], sprintf("%.4f", values[i]), sep = "\t"), con)
  }
}
...
write_labels <- function(path) {
  ...
  writeLines("SEPARATOR TAB", con)
  writeLines("DATA", con)
  for (i in seq_along(tree_tips)) writeLines(paste(ann$sample_id[i], ann$tip_label_closest_taxon[i], sep = "\t"), con)
}
...
write_colorstrip(file.path(out_dir, "iupac_blast_closest_taxon_colorstrip.txt"), "Closest BLAST taxon", ann$best_reference_taxon, taxon_colors)
write_colorstrip(file.path(out_dir, "iupac_blast_closest_genus_colorstrip.txt"), "Closest BLAST genus", ann$closest_genus, genus_colors)
write_colorstrip(file.path(out_dir, "iupac_blast_confidence_colorstrip.txt"), "BLAST assignment confidence", ann$blast_confidence, confidence_colors)
write_gradient(file.path(out_dir, "iupac_blast_percent_identity_gradient.txt"), "BLAST percent identity", ann$percent_identity, "#fff7bc", "#7f0000")
write_gradient(file.path(out_dir, "iupac_blast_query_coverage_gradient.txt"), "BLAST query coverage", ann$query_coverage_percent, "#edf8fb", "#006d2c")
write_gradient(file.path(out_dir, "iupac_blast_reference_coverage_gradient.txt"), "BLAST reference coverage", ann$reference_coverage_percent, "#f7fcfd", "#084081")
write_labels(file.path(out_dir, "iupac_blast_taxon_tip_labels.txt"))

write.table(ann, file.path(out_dir, "iupac_blast_taxon_annotation_table.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)

summary_table <- data.frame(
  metric = c(
    "tree_file", "blast_file", "tree_tips", "tips_with_blast_assignment",
    "tips_without_blast_assignment", "unique_closest_taxa", "unique_closest_genera",
    "mean_percent_identity", "min_percent_identity", "max_percent_identity",
    "mean_query_coverage_percent", "min_query_coverage_percent", "max_query_coverage_percent"
  ),
  value = c(
    tree_file, blast_file, length(tree_tips), sum(ann$in_blast_table == "yes"),
    sum(ann$in_blast_table == "no"), length(unique(ann$best_reference_taxon[!is.na(ann$best_reference_taxon)])),
...
write.table(summary_table, file.path(out_dir, "iupac_blast_taxon_annotation_summary.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
