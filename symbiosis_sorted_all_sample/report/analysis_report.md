...
This pipeline should be interpreted as a **reference-guided**, target-filtered nif/nod phylogenetic workflow. Several caveats are important for interpreting the V2 nifH trees. First, the MC negative-control information is limited to two samples, MC-1 and MC-2, so MC-based filtering should be treated conservatively; targets with no good-coverage MC signal should be described as having no evidence of MC signal in the available controls, not as definitively contamination-free. Second, mixed-IUPAC calls are threshold-based. In this analysis, a mixed site required total depth >= 20 reads, minor allele count >= 5 reads, and minor allele fraction >= 0.20. Third, mixed-IUPAC signal is consistent with possible multiple templates or multicopy signal, but it is not direct proof of multiple nitrogen-fixing organisms, because mixed signal could also arise from mapping ambiguity among similar references, PCR/sequencing artifacts, or low-confidence regions. Finally, tree tips represent sample-target sequences, not unique biological samples. A single biological sample can appear more than once if it has reliable coverage for multiple nifH target references; therefore, the 1,802 strict-tree tips and 4,212 mixed-IUPAC-tree tips should not be interpreted as 1,802 or 4,212 independent samples.

For the mixed-IUPAC tree, we did not run the full exhaustive/slow IQ-TREE setting. To reduce time complexity, we used the best-fit model identified from the strict single-dominant analysis (GTR+F+R9) and built a faster exploratory mixed-IUPAC tree without full ModelFinder search or bootstrap support. Therefore, the mixed-IUPAC tree should be interpreted as a sensitivity/exploratory tree showing how mixed-template sequences behave, while the strict single-dominant tree is the more conservative primary tree.
