...
# Step 25: Create iTOL BLAST/taxon annotations for the mixed-IUPAC nm5000 nifH tree.
#
# Tree:
#   result/trees/nifH_consensus50_iupac_all_pass_nm5000.treefile
#
# Required BLAST best-hit table:
#   result/tables/iupac_nifH_best_reference_hit_with_taxon.tsv
#
...
tree_file <- file.path(project_dir, "result/trees/nifH_consensus50_iupac_all_pass_nm5000.treefile")
blast_file <- file.path(project_dir, "result/tables/iupac_nifH_best_reference_hit_with_taxon.tsv")
out_dir <- file.path(project_dir, "result/itol_annotations_blast_taxon_iupac_nm5000")
...
stop_if_missing(tree_file, "Mixed-IUPAC nm5000 tree file")
...
write_colorstrip(
  file.path(out_dir, "iupac_nm5000_blast_closest_taxon_colorstrip.txt"),
  "Closest BLAST taxon",
  ann$best_reference_taxon,
  taxon_colors
)
write_colorstrip(
  file.path(out_dir, "iupac_nm5000_blast_closest_genus_colorstrip.txt"),
  "Closest BLAST genus",
  ann$closest_genus,
  genus_colors
)
write_colorstrip(
  file.path(out_dir, "iupac_nm5000_blast_confidence_colorstrip.txt"),
  "BLAST assignment confidence",
  ann$blast_confidence,
  confidence_colors
)
write_gradient(
  file.path(out_dir, "iupac_nm5000_blast_percent_identity_gradient.txt"),
...
write_gradient(
  file.path(out_dir, "iupac_nm5000_blast_query_coverage_gradient.txt"),
...
write_gradient(
  file.path(out_dir, "iupac_nm5000_blast_reference_coverage_gradient.txt"),
  "BLAST reference coverage",
  ann$reference_coverage_percent,
  "#f7fcfd",
  "#084081"
)
write_labels(file.path(out_dir, "iupac_nm5000_blast_taxon_tip_labels.txt"))

write.table(
  ann,
  file.path(out_dir, "iupac_nm5000_blast_taxon_annotation_table.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  summary_table,
  file.path(out_dir, "iupac_nm5000_blast_taxon_annotation_summary.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
