...
# nolG: extract strict and mixed-IUPAC consensus sequences from good-coverage BAMs.
# Good coverage is selected from consensus50_gene_coverage_all_samples.tsv:
#   gene == nolG, percent_covered >= 80, mean_depth >= 10.
...
PYTHON_SCRIPT="$SCRIPT_DIR/01_extract_nolG_consensus_consensus50.py"
...
STRICT_FASTA="$RUN_DIR/nolG_consensus50_strict_single_dominant_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
IUPAC_FASTA="$RUN_DIR/nolG_consensus50_iupac_all_pass_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
QC_TABLE="$RUN_DIR/nolG_consensus_qc.tsv"
SELECTED="$RUN_DIR/nolG_selected_samples.tsv"
...
echo "nolG: selected samples: $N"
echo "nolG: output: $RUN_DIR"
