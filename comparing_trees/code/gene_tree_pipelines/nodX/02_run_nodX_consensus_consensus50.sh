...
# nodX: extract strict and mixed-IUPAC consensus sequences from good-coverage BAMs.
# Good coverage is selected from consensus50_gene_coverage_all_samples.tsv:
#   gene == nodX, percent_covered >= 80, mean_depth >= 10.
...
PYTHON_SCRIPT="$SCRIPT_DIR/01_extract_nodX_consensus_consensus50.py"
...
STRICT_FASTA="$RUN_DIR/nodX_consensus50_strict_single_dominant_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
IUPAC_FASTA="$RUN_DIR/nodX_consensus50_iupac_all_pass_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
QC_TABLE="$RUN_DIR/nodX_consensus_qc.tsv"
SELECTED="$RUN_DIR/nodX_selected_samples.tsv"
...
echo "nodX: selected samples: $N"
echo "nodX: output: $RUN_DIR"
