...
# nodX: BLAST strict and mixed-IUPAC sample consensus sequences against nodX taxon references.
...
RUN_DIR="$OUT/gene_trees/nodX/01_consensus"
BLAST_DIR="$OUT/gene_trees/nodX/05_blast_taxon_annotation"
MIN_PCT="${MIN_PCT:-80}"
...
REF=""
for candidate in "$TAXON_REF_DIR/nodX.fa" "$TAXON_REF_DIR/nodX.fasta" "$TAXON_REF_DIR/nodX.fa.aln" "$TAXON_REF_DIR/nodX.aln"; do
  if [[ -s "$candidate" ]]; then REF="$candidate"; break; fi
done
if [[ -z "$REF" ]]; then echo "ERROR: no taxon reference found for nodX in $TAXON_REF_DIR" >&2; exit 1; fi
...
REF_UNGAP="$BLAST_DIR/nodX_taxon_reference_ungapped.fasta"
ungap_fasta "$REF" > "$REF_UNGAP"
makeblastdb -in "$REF_UNGAP" -dbtype nucl -out "$BLAST_DIR/nodX_taxon_ref_db" >/dev/null

for SET in strict iupac; do
  if [[ "$SET" == "strict" ]]; then
    QUERY="$RUN_DIR/nodX_consensus50_strict_single_dominant_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
  else
    QUERY="$RUN_DIR/nodX_consensus50_iupac_all_pass_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
  fi
  if [[ ! -s "$QUERY" ]]; then echo "Skipping $SET; query missing: $QUERY"; continue; fi

  QUERY_UNGAP="$BLAST_DIR/nodX_${SET}_query_ungapped.fasta"
  BLAST_TSV="$BLAST_DIR/nodX_${SET}_blast.tsv"
  BEST_TSV="$BLAST_DIR/nodX_${SET}_best_reference_hit_with_taxon.tsv"
  ungap_fasta "$QUERY" > "$QUERY_UNGAP"
  blastn -query "$QUERY_UNGAP" -db "$BLAST_DIR/nodX_taxon_ref_db"     -out "$BLAST_TSV"     -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore'     -evalue "$EVALUE" -num_threads "$THREADS" -max_target_seqs 10
