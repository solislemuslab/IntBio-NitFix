...
# nifD: align strict and mixed-IUPAC consensus FASTA files with MAFFT.
...
ALIGN_DIR="${OUT}/gene_trees/nifD/02_alignment"
mkdir -p "$ALIGN_DIR"
...
for SET in strict_single_dominant iupac_all_pass; do
  if [[ "$SET" == "strict_single_dominant" ]]; then
    IN="$RUN_DIR/nifD_consensus50_strict_single_dominant_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
    OUTFA="$ALIGN_DIR/nifD_consensus50_strict_single_dominant.mafft.fasta"
  else
    IN="$RUN_DIR/nifD_consensus50_iupac_all_pass_pct${MIN_PCT}_depth${MIN_MEAN_DEPTH}_Nle${MAX_N_PERCENT}.fasta"
    OUTFA="$ALIGN_DIR/nifD_consensus50_iupac_all_pass.mafft.fasta"
  fi
  ...
  echo "Aligning nifD $SET ($(grep -c '^>' "$IN") sequences)"
