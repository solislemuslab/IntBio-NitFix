...
# nolF: build strict single-dominant IQ-TREE with ModelFinder, UFBoot, SH-aLRT, nm5000.
...
GENE="nolF"
ALIGN="${OUT}/gene_trees/nolF/02_alignment/nolF_consensus50_strict_single_dominant.mafft.fasta"
TREE_DIR="${OUT}/gene_trees/nolF/03_tree_strict_nm5000"
PREFIX="$TREE_DIR/nolF_consensus50_strict_single_dominant_nm5000"
FORCE="${FORCE:-0}"
mkdir -p "$TREE_DIR"
...
"$IQTREE"   -s "$ALIGN"   -st DNA   -m MFP   -B 1000   -alrt 1000   -nm 5000  -safe \
  -T AUTO   --prefix "$PREFIX"   | tee "$PREFIX.run.log"
