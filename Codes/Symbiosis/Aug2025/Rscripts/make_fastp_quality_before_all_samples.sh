...
OUT = QC_DIR / "fastp_quality_before_trimming_all_samples_lightpurple_mean_blue.svg"
...
r1_mean = mean_curve(r1_before_all)
r2_mean = mean_curve(r2_before_all)
...
def add_legend(svg, x, y):
    svg.append(f'<line x1="{x}" y1="{y}" x2="{x+40}" y2="{y}" stroke="#9E9AC8" stroke-width="2"/>')
    ...
    svg.append(f'<text x="{x+48}" y="{y+4}" font-size="13" font-family="Arial">mean across all samples</text>')
    y += 20
...
svg.append('<text x="550" y="35" text-anchor="middle" font-size="24" font-family="Arial" font-weight="bold">Per-base Quality Profiles Before Trimming</text>')
svg.append(f'<text x="550" y="62" text-anchor="middle" font-size="14" font-family="Arial">Light purple lines = individual samples; blue line = mean across {len(json_files)} samples</text>')

draw_axes(svg, "A. Read 1 before trimming", 90, 120, 420, 280)
draw_axes(svg, "B. Read 2 before trimming", 620, 120, 420, 280)

for curve in r1_before_all:
    svg.append(f'<polyline points="{curve_points(curve, 90, 120, 420, 280)}" fill="none" stroke="#9E9AC8" stroke-width="0.55" opacity="0.42"/>')

for curve in r2_before_all:
    svg.append(f'<polyline points="{curve_points(curve, 620, 120, 420, 280)}" fill="none" stroke="#9E9AC8" stroke-width="0.55" opacity="0.42"/>')

svg.append(f'<polyline points="{curve_points(r1_mean, 90, 120, 420, 280)}" fill="none" stroke="#0072B2" stroke-width="3"/>')
svg.append(f'<polyline points="{curve_points(r2_mean, 620, 120, 420, 280)}" fill="none" stroke="#0072B2" stroke-width="3"/>')

add_legend(svg, 410, 470)
svg.append('</svg>')
